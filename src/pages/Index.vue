...
<template>
  ...
  <div class="max-w-7xl mx-auto p-6 sm:px-8 flex flex-col items-stretch gap-2">
              ...
              >
                <div class="flex items-center justify-between gap-3 mb-2">
                  <h5 class="text-lg font-medium">
                    ...
                    />
                    <bi-motherboard
                      v-else-if="stat.type === 'Board'"
                      class="inline-block w-5 h-5 mr-2 text-gray-600 dark:text-gray-400"
                    />
                    <bi-router
                      v-else-if="stat.type === 'Router'"
                      class="inline-block w-5 h-5 mr-2 text-gray-600 dark:text-gray-400"
                    />
                    <bi-hdd-network
                      v-else-if="stat.type === 'KVM'"
                      class="inline-block w-5 h-5 mr-2 text-gray-600 dark:text-gray-400"
                    />
                    <bi-hdd-stack
                      v-else
                ...
                >
                  <p class="inline-block">
                    <bi-geo-alt
                      class="inline-block w-4 h-4 mr-2 text-gray-400 dark:text-gray-500"
                    />
                    <span
                      class="align-middle"
                      v-text="stat.location || 'Unknown'"
                    />
                  </p>
                  <template v-if="isOnline(stat, stats.updated)">
                    <p class="inline-block">
                      ...
                      <span class="align-middle"
                        ><span class="hidden sm:inline">Uptime: </span
                        >{{ stat.uptime }}</span
                      >
                    </p>
                ...
                >
                  ...
                  <div class="flex items-center gap-2">
                    <bi-cpu
                      class="inline-block w-4 h-4 text-gray-400 dark:text-gray-500"
                    />
                    <div class="flex-1 flex flex-col items-stretch gap-1">
                      ...
                      >
                        <span>{{ stat.cpu }}%</span>
                        <span v-if="stat.process_count || stat.thread_count"
                          >{{ stat.process_count }} Processes /
                  ...
                  <div v-if="stat.memory_total" class="flex items-center gap-2">
                    ...
                    <div class="flex-1 flex flex-col items-stretch gap-1">
                      ...
                      >
                        ...
                        <span
                          >{{ getMemoryText(stat) }}
                          <span v-if="stat.swap_total" class="hidden sm:inline"
                            >({{ getSwapText(stat) }} swap)</span
                          ></span
                        >
                      </div>
                      <div
                        class="flex w-full bg-gray-200 rounded-full h-1 dark:bg-gray-700"
                      >
                        <div
                  ...
                  <!-- Network -->
                  <div
                    class="lg:col-span-3 flex items-center gap-2 text-xs text-gray-600 dark:text-gray-300"
                  >
                    <bi-ethernet
                      class="inline-block w-4 h-4 text-gray-400 dark:text-gray-500"
                    />
                    <div
                      class="grid grid-flow-col grid-rows-2 lg:grid-rows-none gap-x-2"
                    >
                      <p class="inline-block">
                        <bi-arrow-up-short
                          class="inline-block w-4 h-4 text-gray-400 dark:text-gray-500"
                      ...
                      <p class="inline-block">
                        <span
                          class="ml-1 align-middle text-gray-400 dark:text-gray-500"
                          >TCP:</span
                        >
    ...
    >
      <svg
        aria-hidden="true"
        class="w-8 h-8 text-gray-300 animate-spin dark:text-gray-700 fill-blue-600"
        viewBox="0 0 100 101"
        fill="none"
    ...
    >
      <div
        v-if="!stats.servers.length"
        class="flex flex-col items-center gap-4 py-8 text-lg text-gray-600 dark:text-gray-300"
      >
