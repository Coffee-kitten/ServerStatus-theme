  ...
  <div class="max-w-7xl mx-auto p-6 sm:px-8 flex flex-col items-stretch gap-2">
    <template v-if="stats">
      <template v-for="(stat0, index) of stats.servers">
        <details v-if="index === 0 || stat0.gid !== stats.servers[index - 1].gid" class="collapse bg-base-200">
          <summary class="collapse-title text-xl font-medium">
            {{ stat0.gid }}
          </summary>
          <div class="collapse-content">
            <div class="grid lg:grid-cols-2 gap-6 mb-4 items-stretch gap-6 mb-4">
              <template v-for="stat of stats.servers" :key="stat.name">
                <div v-if="stat.gid === stat0.gid"
                  class="p-4 border border-slate-200 dark:border-[#1e222600] rounded-lg shadow-lg bg-slate-100 dark:bg-[#1e222600]">
                  <div class="flex items-center justify-between gap-3 mb-2">
                    <h5 class="text-lg font-medium">
                      <bi-display v-if="stat.type === 'PC'"
                        class="inline-block w-5 h-5 mr-2 text-gray-600 dark:text-gray-400" />
                      <bi-motherboard v-else-if="stat.type === 'Board'"
                        class="inline-block w-5 h-5 mr-2 text-gray-600 dark:text-gray-400" />
                      <bi-router v-else-if="stat.type === 'Router'"
                        class="inline-block w-5 h-5 mr-2 text-gray-600 dark:text-gray-400" />
                      <bi-hdd-network v-else-if="stat.type === 'KVM'"
                        class="inline-block w-5 h-5 mr-2 text-gray-600 dark:text-gray-400" />
                    ...
                    <div class="flex items-center gap-2">
                      <span v-if="!isOnline(stat, stats.updated)"
                        class="inline-block w-16 py-1 text-center text-xs font-semibold text-red-600 bg-red-100 dark:text-red-100 dark:bg-red-600 rounded-full">Offline</span>
                      <span v-else
                  ...
                  <div class="flex items-center gap-5 text-sm text-gray-600 dark:text-gray-300">
                    ...
                    <template v-if="isOnline(stat, stats.updated)">
                      <p class="inline-block">
                        ...
                        <span class="align-middle"><span class="hidden sm:inline">Uptime: </span>{{ stat.uptime
                          }}</span>
                      </p>
                      <p class="hidden sm:inline-block">
                        <bi-gear class="inline-block w-4 h-4 mr-2 text-gray-400 dark:text-gray-500" />
                        <span class="align-middle">Load:
                          <span :class="{
                            'text-red-600 dark:text-red-500': stat.load_1 > 3,
                          }">{{ stat.load_1 }}</span></span>
                      </p>
                    </template>
                  </div>

                  <div v-if="isOnline(stat, stats.updated)" class="flex flex-col mt-3 gap-3">
                    <!-- CPU -->
                    <div class="flex items-center gap-2">
                      <bi-cpu class="inline-block w-4 h-4 text-gray-400 dark:text-gray-500" />
                      <div class="flex-1 flex flex-col items-stretch gap-1">
                        <div
                          class="flex flex-row-reverse justify-between items-center text-xs text-gray-600 dark:text-gray-300">
                          <span>{{ stat.cpu }}%</span>
                          <span v-if="stat.process_count || stat.thread_count">{{ stat.process_count }} Processes /
                            {{ stat.thread_count }} Threads</span>
                          <span v-else class="text-gray-400 dark:text-gray-500">CPU Usage</span>
                        </div>
                        <div class="w-full bg-gray-200 rounded-full h-1 dark:bg-gray-700">
                          <div class="h-1 rounded-full transition-[width] duration-500 ease-linear" :class="{
                            'bg-blue-600 dark:bg-blue-500': stat.cpu < 80,
                            'bg-red-600 dark:bg-red-500': stat.cpu >= 80,
                      ...
                      <div class="flex-1 flex flex-col items-stretch gap-1">
                        <div
                          ...
                          <span>{{ getMemoryText(stat) }}
                            <span v-if="stat.swap_total" class="hidden sm:inline">({{ getSwapText(stat) }}
                              swap)</span></span>
                        </div>
                        <div class="flex w-full bg-gray-200 rounded-full h-1 dark:bg-gray-700">
                          <div class="h-1 transition-[width] duration-500 ease-linear" :class="{
                            'bg-blue-600 dark:bg-blue-500':
                              getMemoryPercentage(stat) < 80,
                            'bg-red-600 dark:bg-red-500':
                              getMemoryPercentage(stat) >= 80,
                            'rounded-s-full': stat.swap_total,
                            'rounded-full': !stat.swap_total,
                    ...
                    <div v-if="stat.hdd_total" class="flex items-center gap-2">
                      ...
                      <div class="flex-1 flex flex-col items-stretch gap-1">
                        ...
                        <div class="w-full bg-gray-200 rounded-full h-1 dark:bg-gray-700">
                          <div class="h-1 rounded-full transition-[width] duration-500 ease-linear" :class="{
                            'bg-blue-600 dark:bg-blue-500':
                              stat.hdd_total - stat.hdd_used > 1024,
                            'bg-red-600 dark:bg-red-500':
                              stat.hdd_total - stat.hdd_used <= 1024,
    ...
    <div v-if="!stats && isLoading" class="flex justify-center items-center gap-3 py-8" role="status">
      <svg aria-hidden="true" class="w-8 h-8 text-gray-300 animate-spin dark:text-gray-700 fill-blue-600"
    ...
    <div v-else-if="stats && !stats.servers.length" class="grid lg:grid-cols-2 gap-6 mb-4 items-stretch gap-6">
      <div class="flex flex-col items-center gap-4 py-8 text-lg text-gray-600 dark:text-gray-300">
